module hazard_unit (
    // Load-Use 检测所需的输入
    input  [4:0] rs1_id,       // 正在 ID 阶段解码的源寄存器 1
    input  [4:0] rs2_id,       // 正在 ID 阶段解码的源寄存器 2
    input  [4:0] rd_ex,        // 上一条指令（在 EX 阶段）的目标寄存器
    input        mem_read_ex,  // EX 阶段是否是 Load 指令 (读取内存)
    
    // 控制冒险检测所需的输入
    input        pc_src_ex,    // EX 阶段决定的跳转信号 (1表示跳转 Taken)
    
    // 输出控制信号
    output reg   stall_if_id,  // 冻结 PC 和 IF/ID 寄存器
    ...
    always @(*) begin
        ...
        if (mem_read_ex && ((rd_ex == rs1_id) || (rd_ex == rs2_id))) begin
            stall_if_id = 1'b1; // 暂停 PC 更新，暂停 IF/ID 写入
